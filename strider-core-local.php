...
abstract class strider_core_b2_LogDeprecatedCalls {

	public $sc_data_name = 'plugin_strider_core_b2_LogDeprecatedCalls_data';
	public $sc_option_name = 'plugin_strider_core_b2_LogDeprecatedCalls_options';
...
// the rest only runs once
		if ( $strider_core_b2_firstrundone ) {
			return true;
		}
		$strider_core_b2_firstrundone = true;

		$this->core_activate();
//$this->version_check();  //  non-functional
...
// $param == 0'basefile.php', 1'Title', 2'Menu Text', 3'permission', 4'page_tag', 5'function', 6'menu_icon'
		if ( current_user_can( $param[ 3 ] ) ) {
			...
			$page = add_submenu_page( $param[ 0 ], $param[ 1 ], $icon . $param[ 2 ], $param[ 3 ], $param[ 4 ],
			                          array( &$this, $param[ 5 ] ) );
...
// Add homepage link to settings page footer
		...
		printf( $this->sc__( '%1$s plugin | <span title="Strider Core version %2$s">Version %3$s</span> | by %4$s<br />' ),
		        $pluginfo[ 'Title' ], $strider_core_b2_info[ 'version' ], $pluginfo[ 'Version' ],
		        $pluginfo[ 'Author' ] );
	}
...
// explicitly set all checkboxes true or false
		...
		echo '<div id="message" class="updated fade"><p><strong>' . __( 'Settings saved.' ) . '</strong></p>' .
		     $message . '</div>';
...
//  IMPORTANT: This doesn't do anything unless the plugin has a "Version Check URI" header, but the code must remain here because it's part of a common framework that SOME plugins do use.
	...
	function filter_set_update_plugins( $data ) {
		...
		*/
		...
		if ( version_compare( $newitem->new_version,
		                      $this->get_plugin_data( 'Version', WP_PLUGIN_DIR . '/' . $plugin ) ) == 1
		) {
	...
	function plugin_update_rows() {
		...
		foreach ( $names as $file ) {
			if ( version_compare( $sc_data[ 'updates' ][ $file ]->new_version, $this->get_plugin_data( 'Version' ) ) !=
			     1
			) {
	...
	function plugin_update_row( $plugin_file, $plugin_data, $context ) {
		...
		printf( $this->sc__( 'Version %1$s of %2$s is available for <a href="%3$s" title="%4$s">download</a>.' ),
		        $plugin->new_version, $plugin_data[ 'Name' ], $plugin_data[ 'PluginURI' ],
		        $this->sc__( 'visit plugin homepage' ) );
		echo '</div></td></tr>';
	}
...
//***********************
	...
	protected function _set_default_options( $def_options, $mode = 'merge', $curr_options = null ) {
		_deprecated_function( __FUNCTION__, 'Strider Core 0.2 beta 2',
		                      'get_default_options()  (NOTE: See changes to get/set_default_options in the code. Probably rename your function to get_default_actions)' );
		if ( $mode == 'reset' ) {
			delete_option( $this->option_name );
		};
